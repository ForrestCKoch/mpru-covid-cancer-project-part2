...
disp %>% filter(is.na(class)) %>% 
  ...
  ungroup() %>% unique %>% 
  inner_join(.,load.drugmap(paste0('../',drugmap.path)) %>% 
               select(ITEM_CODE, DRUG_NAME, FORM.STRENGTH),by='ITEM_CODE') %>%
  select(c('DRUG_NAME','ATC_CODE','FORM.STRENGTH','ITEM_CODE','First','Last')) %>% datatable #write.csv(.,'../results/l01-missing-classification.csv',row.names=F) 
```
